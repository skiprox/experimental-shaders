...
float scene(vec3 pos) {
    ...
    pos1b.y += .1 + sin(u_time + random(i.xz) * 6.2) * 0.05;
    pos1b.x +=  sin(u_time + random(i.xz+1.0) * 6.2) * 0.05;
    pos1b.z +=  sin(u_time + random(i.xz+2.0) * 6.2) * 0.05;
...
vec3 trace(vec3 camOrigin, vec3 dir, out float totalDist) {
    // any bigger than 32 and it starts freaking out
    const int maxSteps = 32;
    // We want to project from our camera,
    // so that's where we start our ray
    vec3 ray = camOrigin;
    totalDist = 0.0;
    ...
    for(int i = 0; i < maxSteps; i ++ ) {
        // distance is calculated based on what is in our scene
        ...
        if (abs(dist) < 0.001) {
            ...
            float s = jsoftshadow(lightOrigin, ray, 0.01);
            diffuse *= s;
...
void main() {
    ...
    // Normal camera stuff here
    vec3 camOrigin = vec3(0.0, 0.5, - 3.0);
    vec3 rayOrigin = vec3(camOrigin.xy + uv, camOrigin.z + 3.0);
    
    // This is the base color, all other elements
    // colors get added to this to create the scene
    ...
    for (float pass = 0.; pass < passes; pass++){
        ...
        float dist;
        // Get our "color" by running the trace function,
        // which bounces particles off objects in our scene
        // and returns a color to use
        vec3 c = trace(camOrigin, dir, dist);
    
        float fog = clamp(map(dist, 6.0, 7.0, 1.0, 0.0), 0.0, 1.0);
        // first part of this mix is the sky color,
        // second part is the color returned by `trace` function
